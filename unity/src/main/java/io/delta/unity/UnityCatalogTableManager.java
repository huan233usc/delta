...
/**
 * Unity Catalog implementation of DeltaTableManager.
...
public class UnityCatalogTableManager implements DeltaTableManager {
    ...
    public void initialize(String tablePath, Map<String, String> properties) {
        ...
        if (ucTableId == null || ucTableId.trim().isEmpty()) {
            throw new IllegalArgumentException("UC table ID not found in properties. " +
                "Expected key: " + UCCommitCoordinatorClient.UC_TABLE_ID_KEY);
        }
    ...
    public Snapshot update() {
        Snapshot snapshot = ccv2Client.loadSnapshot(
            kernelEngine, ucTableId, tablePath, Optional.empty());
    ...
    public DeltaHistoryManager.Commit getActiveCommitAtTime(
        ...
        GetCommitsResponse response = ccv2Client.getRatifiedCommitsFromUC(
            ucTableId, tablePath, Optional.empty());
    ...
    public void checkVersionExists(
        Long version, 
        Boolean mustBeRecreatable,
        Boolean allowOutOfRange) throws AnalysisException {
        ...
        long earliest = mustBeRecreatable 
            ? DeltaHistoryManager.getEarliestRecreatableCommit(kernelEngine, snapshot.getLogPath(), earliestRatifiedVersion)
            : DeltaHistoryManager.getEarliestDeltaFile(kernelEngine, snapshot.getLogPath(), earliestRatifiedVersion);
    ...
    private String extractCatalogName(Map<String, String> properties) {
        ...
        throw new IllegalArgumentException("Cannot determine Unity Catalog name from properties. " +
    ...
    private UriAndToken extractUCCredentials(String catalogName, Map<String, String> properties) {
        ...
        if (uri == null || uri.trim().isEmpty()) {
            throw new IllegalArgumentException("UC URI not found for catalog: " + catalogName + 
                ". Expected property: " + uriKey);
        }
        
        if (token == null || token.trim().isEmpty()) {
            throw new IllegalArgumentException("UC token not found for catalog: " + catalogName + 
                ". Expected property: " + tokenKey);
        }
    ...
    private Engine createEngine(Map<String, String> properties) {
        ...
        properties.entrySet().stream()
            ...
            .forEach(entry -> {
                String key = entry.getKey().startsWith("hadoop.") 

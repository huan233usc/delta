...
/**
 * Factory for creating DeltaTableManager instances.
 * Enhanced version supporting dynamic class loading and custom implementations,
 * following Iceberg's proven factory pattern.
 */
public class DeltaTableManagerFactory {
    ...
    private static final String PATH_IMPL = "io.delta.kernel.spark.catalog.utils.PathBasedTableManager";

    /**
     * Creates the appropriate DeltaTableManager based on the catalog table configuration.
     * This is the main entry point for compatibility with existing code.
     * 
    ...
    private static Map<String, String> extractPropertiesFromCatalogTable(CatalogTable catalogTable) {
        ...
        JavaConverters.mapAsJavaMap(sparkConf).entrySet().stream()
            .filter(entry -> entry.getKey().startsWith("spark.sql.catalog.") ||
     ...
     */
    private static String detectManagerType(CatalogTable catalogTable, Map<String, String> properties) {
        ...
        switch (managerType.toLowerCase()) {
            ...
            default:
                throw new UnsupportedOperationException(
                    "Unknown table manager type: " + managerType + 
                    ". Supported types: unity, path. " +
                    "For custom implementations, use " + DELTA_TABLE_MANAGER_IMPL + " property.");
        }
    }
    
    /**
     * Load table manager using dynamic class loading.
     * Supports both built-in implementations and custom ones.
     */
    public static DeltaTableManager loadTableManager(
        String impl, CatalogTable catalogTable, Map<String, String> properties) {
        
        if (impl == null) {
            throw new IllegalArgumentException("Cannot initialize DeltaTableManager, impl class name is null");
        }
        ...
        try {
            ...
            if (!DeltaTableManager.class.isAssignableFrom(clazz)) {
                throw new IllegalArgumentException(
                    String.format("Cannot initialize DeltaTableManager, %s does not implement DeltaTableManager.", impl));
            }
            ...
            throw new IllegalArgumentException(
                String.format("DeltaTableManager implementation %s must have a no-arg constructor", impl), e);
        } catch (Exception e) {

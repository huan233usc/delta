...
public interface Transaction {
  ...
  SnapshotStateBuilder getBaseSnapshotState();
  /**
   ...
   *     { Transaction#generateAppendActions(Engine, Row, CloseableIterator, DataWriteContext)}. The
   *     {@link CloseableIterable} allows the Kernel to access the list of actions multiple times
   *     (in case of retries to resolve the conflicts due to other writers to the table). Kernel
   *     provides a in-memory based implementation of {@link CloseableIterable} with utility API
   *     {@link CloseableIterable#inMemoryIterable(CloseableIterator)}
  ...
  static CloseableIterator<Row> generateAppendActions(
      ...
      SnapshotStateBuilder snapshotStateBuilder) {
    checkArgument(
        ...
        dataFileStatus -> {
          ...
          AddFile addFileRow =
              AddFile.convertDataFileStatus(
                  ...
                  true /* dataChange */);
          snapshotStateBuilder.addFile(addFileRow);
